...
if (import.meta.vitest) {
	...
	describe('nested file paths', () => {
		it.each([
			...
			'extracts valid nested file paths',
			(requestAssetUrl, fileAsset) => {
				//
				const matchedFilename = nestedFilePathMatcher.exec(
					requestAssetUrl.slice(1),
				)
				expect(matchedFilename).toBeTruthy()
				expect(matchedFilename?.[0]).toBe(fileAsset)
			},
		...
		it.each([
			...
			'ignores invalid nested file paths',
			(invalidRequestAssetUrl) => {
				const matchedFilename = nestedFilePathMatcher.exec(
					invalidRequestAssetUrl.slice(1),
				)
